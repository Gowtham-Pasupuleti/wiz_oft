...
function DropDownFinal() {
  ...
  return (
    <>
      <div
        className={`inline-block text-sm items-center bg-[#5C20CF]  py-5 px-3 hover:bg-[#5317C3] `}
        ref={ref}
      >
        <Menu>
          <div >
            <Menu.Button
              className={`flex items-center font-light text-white outline-none `}
             onClick={() => setClicked(!clicked)}
            >
              <SettingsIcon />
              Manage
        ...
        transition duration-150 ease-in-out ${clicked ? "rotate-180" : ""}`}
          ...
          <Menu.Items
            className=
  {`fixed flex flex-col w-40
  border-white font-medium text-base font-serif mt-8
