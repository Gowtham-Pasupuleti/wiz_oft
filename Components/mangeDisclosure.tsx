      ...
      <div className="" ref={ref}>
        <Disclosure>
          {({ open }) => (
            <>
            <div className=''>
            <Disclosure.Button onClick={() => setClicked(!clicked)} className="flex  items-center w-full bg-[#5C20CF] text-white hover:bg-[#5317C3] py-4 px-5 font-poppins text-sm">
            <SettingsIcon />
             <span>Manage</span>
            ...
            {clicked && (<Disclosure.Panel static className={`px-4 pt-2 pb-2 text-sm text-white `}>
            {({ close }) => (


                <ul onClick={() => close(ref)}>
            ...
            <AccessControlIcon />
             ...
             <svg className={`fill-current h-3 w-3 transform transition duration-500 ease-in-out ${open ? 'rotate-180' : ''}`}
              ...
              <Disclosure.Panel className="px-4 pt-4 pb-2 text-sm text-white">
                <ul>
                <Link href="/configureUser"><li className='px-5 pt-2 pb-3 flex items-center hover:bg-[#5317C3]'><Partners/>&nbsp;Configure User</li></Link>
                <Link href="/ConfigureGroup"><li className='px-5 pt-2 pb-3 flex items-center hover:bg-[#5317C3]'><Customers/>&nbsp;Configure Group</li></Link>
                </ul>
              </Disclosure.Panel>
              </>)}
